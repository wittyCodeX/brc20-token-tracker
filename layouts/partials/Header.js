...
const Header = () => {
  ...
  return (
    <header className="header w-full fixed shadow-md">
                ...
                <React.Fragment key={`menu-${i}`}>
                  {menu.hasChildren ? (
                      ...
                      <ul className="nav-dropdown-list hidden transition-all duration-300 group-hover:top-[46px] group-hover:block md:invisible md:absolute md:top-[60px] md:block md:opacity-0 md:group-hover:visible md:group-hover:opacity-100">
                        {menu.children.map((child, i) => (
                          ...
                          >
                            <Link
                              href={child.url}
                              className={`nav-dropdown-link block  font-extrabold ${router.asPath === child.url && "active"
                                }`}
                            >
                              {child.name}
                            </Link>
                    ...
                    <li className="nav-item">
                      <Link
                        href={menu.url}
                        className={`nav-link block font-extrabold text-base ${router.asPath === menu.url && "active"
                          }`}
                      >
                        {menu.name}
                      </Link>
                    ...
                    <li className="nav-item">
                      <Link href='' onClick={() => Inscribe()} className={`nav-link block font-extrabold text-base ${router.asPath === menu.url && "active"
                        }`}>{menu.name}</Link>
                    </li>
                  )
                  }
